...
/*
[img]http://i.imgur.com/Bsxggc1.png[/img]
[img]http://i.imgur.com/b9j4LtE.png[/img]
...
public OnPlayerKeyStateChange(playerid, newkeys, oldkeys){
	...
	if(newkeys & KEY_YES){
		if(creando_actors[playerid]){
			if(IsPlayerInAnyVehicle(playerid))
				return SendClientMessage(playerid,-1,"{919BA4}INFO: no puedes utilizar el editor de actores desde un vehiculo!.");
			if(cantidad_actores>=MAX_ACTORS)
				return SendClientMessage(playerid,-1,"{919BA4}INFO: hay demasiados actores creados hasta el momento!.");

			ShowPlayerDialog(playerid, DIALOG_ACTORS+1, DIALOG_STYLE_MSGBOX, "{FF6800}Sistema de edicion de actores:", "\
			{FFFFFF}¿Que vulnerabilidad deseas aplicarle al actor?", "vulnerable", "invulnerable");
		}
	}
	...
	if(newkeys & KEY_NO){
		if(creando_actors[playerid]){
			...
			if(cantidad_actores<=0)
				return SendClientMessage(playerid,-1,"{919BA4}INFO: no hay ningun actor creado hasta el momento!.");
			...
			cantidad_actores--;
			format(data,sizeof(data),"{EEE019}INFO: haz eliminado un actor correctamente!. actores creados en total: [%d/%d].",cantidad_actores,MAX_ACTORS);
	...
	if(newkeys & KEY_CROUCH && newkeys & KEY_SECONDARY_ATTACK){
		if(creando_actors[playerid]){
			...
			cantidad_actores=0;
			format(data,sizeof(data),"{EEE019}INFO: haz eliminado todos los actores correctamente!. actores creados en total: [%d/%d].",cantidad_actores,MAX_ACTORS);
			SendClientMessage(playerid,-1,data);
		}
	}
	...
	if(newkeys & KEY_CROUCH && newkeys & KEY_SPRINT){
		if(creando_actors[playerid]){
			...
			ShowPlayerDialog(playerid, DIALOG_ACTORS+16, DIALOG_STYLE_MSGBOX, "{FF6800}Sistema de edicion de actores:", "\
			{FFFFFF}elige el tipo de archivo que quieres guardar los actores:", "normal", "filterscript");
		}
	}
    ...
    switch(dialogid){
        ...
        case DIALOG_ACTORS+1:{
	        if(response){
				...
				ShowPlayerDialog(playerid, DIALOG_ACTORS+2, DIALOG_STYLE_MSGBOX, "{FF6800}Sistema de edicion de actores:", "\
				{FFFFFF}¿Que opcion de vida deseas aplicarle al actor?", "personalizar", "por defecto");
	        }else{
        ...
        case DIALOG_ACTORS+2:{
	        if(response){
				ShowPlayerDialog(playerid, DIALOG_ACTORS+3, DIALOG_STYLE_INPUT, "{FF6800}Sistema de edicion de actores:", "\
				{FFFFFF}escribe la cantidad de vida que tendra el actor, debe ser mayor a 0, y menor o igual a 100.\n\
				{A50000}NOTA: puedes utilizar la opcion 'por defecto' que se le aplicara 100 automaticamente. ", "aceptar", "por defecto");
	        }else{
				vida_id = 100.0;
				ShowPlayerDialog(playerid, DIALOG_ACTORS+7, DIALOG_STYLE_MSGBOX, "{FF6800}Sistema de edicion de actores:", "\
				{FFFFFF}¿Que opcion de skin deseas aplicarle al actor?", "personalizar", "por defecto");
	        }
	    }
        ...
        case DIALOG_ACTORS+4:{
	        if(response){
				...
				if(!sscanf(inputtext, "s[100]", opcion)){
					...
					if(archivos){
						...
						if(tipo_archivo_a[playerid]){
							...
							for(new xx=1; xx<cantidad_actores+1; xx++){
								...
								fwrite(archivos, datos);
								fwrite(archivos, "	// NOTA: cambiar los parametros: (Float:fDelta, loop, lockx, locky, freeze, time) \r\n");
							...
							fwrite(archivos, "*/ \r\n\r\n");
							fwrite(archivos, "/**** Sistema de edicion de actores - By OTACON ****/\r\n\r\n");
							fclose(archivos);
						}else{
							fwrite(archivos, "\r\n/**** Sistema de edicion de actores - By OTACON ****/\r\n");
						...
						format(data,sizeof(data),"{005BA1}INFO: haz guardado todos los actores correctamente!. actores creados en total: [%d/%d].",cantidad_actores,MAX_ACTORS);
						SendClientMessage(playerid,-1,data);
						format(data,sizeof(data),"{005BA1}INFO: todos los actores se han guardado en el archivo: '%s.txt', en 'scriptfiles'.",opcion);
						SendClientMessage(playerid,-1,data);
						SendClientMessage(playerid,-1,"{EEE019}INFO: se ha reiniciado el editor, todos los actores han sido eliminados!.");
						for(new xx=1; xx<cantidad_actores+1; xx++){
							if(!IsValidActor(InfoActorEdicion[xx][c_actor]))
								continue;
							DestroyActor(InfoActorEdicion[xx][c_actor]);
					...
					ShowPlayerDialog(playerid, DIALOG_ACTORS+4, DIALOG_STYLE_INPUT, "{FF6800}Sistema de edicion de actores:", "\
					{FFFFFF}escribe un nombre para el archivo que guardara los actores creados.\n\
					{A50000}NOTA: puedes utilizar la opcion 'por defecto' que se le aplicara\n\
					el nombre {FFFFFF}'actors.0001' {A50000}(el numero va contando por archivo creados). ", "aceptar", "por defecto");
				}
	        }else{
				...
				if(archivos){
					...
					format(data,sizeof(data),"{005BA1}INFO: haz guardado todos los actores correctamente!. actores creados en total: [%d/%d].",cantidad_actores,MAX_ACTORS);
					SendClientMessage(playerid,-1,data);
					format(data,sizeof(data),"{005BA1}INFO: todos los actores se han guardado en el archivo: 'actors.%d04.txt', en 'scriptfiles'.",cantidad_archivos);
					SendClientMessage(playerid,-1,data);
					SendClientMessage(playerid,-1,"{EEE019}INFO: se ha reiniciado el editor, todos los actores han sido eliminados!.");
					cantidad_archivos++;
					for(new xx=1; xx<cantidad_actores+1; xx++){
        ...
        case DIALOG_ACTORS+5:{
	        if(response){
				ShowPlayerDialog(playerid, DIALOG_ACTORS+6, DIALOG_STYLE_INPUT, "{FF6800}Sistema de edicion de actores:", "\
				{FFFFFF}escribe el id del actor que quieres establecer:. ", "aceptar", "cancelar");
	        ...
	        if(response){
				...
				if(!sscanf(inputtext, "d", opcion)){
					if( IsValidActor(InfoActorEdicion[opcion][c_actor]) ){
						...
						format(data,sizeof(data),"{FFFFFF}INFO: el id del actor establecido es el: id:%d", id_seleccionado);
						SendClientMessage(playerid,-1,data);
						CallLocalFunction("OnPlayerCommandText", "is", playerid, "/actorfunction");
					}else{
						ShowPlayerDialog(playerid, DIALOG_ACTORS+6, DIALOG_STYLE_INPUT, "{FF6800}Sistema de edicion de actores:", "\
						{FFFFFF}escribe el id del actor que quieres establecer: \n\
						{A50000}NOTA: el id ingresado es incorrecto, intente nuevamente. ", "aceptar", "cancelar");
					}
				}else{
					ShowPlayerDialog(playerid, DIALOG_ACTORS+6, DIALOG_STYLE_INPUT, "{FF6800}Sistema de edicion de actores:", "\
					{FFFFFF}escribe el id del actor que quieres establecer: \n\
					{A50000}NOTA: no haz escrito nada, intente nuevamente. ", "aceptar", "cancelar");
				}
	        }else CallLocalFunction("OnPlayerCommandText", "is", playerid, "/actorfunction");
		}
        ...
        case DIALOG_ACTORS+7:{
	        if(response){
				ShowPlayerDialog(playerid, DIALOG_ACTORS+8, DIALOG_STYLE_INPUT, "{FF6800}Sistema de edicion de actores:", "\
				{FFFFFF}escribe el id del que tendra el actor, debe ser mayor o igual a 0, y menor o igual a 311.\n\
				{A50000}NOTA: puedes utilizar la opcion 'por defecto' que se le aplicara un id aleatorio del 0 al 311. ", "aceptar", "por defecto");
	        }else{
	        	cantidad_actores++;
				InfoActorEdicion[cantidad_actores][a_skin] = 0;
				InfoActorEdicion[cantidad_actores][i_actor] = false;
				...
				format(data,sizeof(data),"{EEE019}INFO: haz creado un actor correctamente!. actores creados en total: [%d/%d].",cantidad_actores,MAX_ACTORS);
				SendClientMessage(playerid,-1,data);
	        }
	    }
	        ...
	        if(response){
				...
				if(!sscanf(inputtext, "s[25]", name_a)){
					if(strlen(name_a)>=4 && strlen(name_a)<=24){
						...
						format(data,sizeof(data),"{FFFFFF}INFO: le haz cambiado el nombre al actor id:%d, correctamente!.", id_seleccionado);
						SendClientMessage(playerid,-1,data);
						format(data,sizeof(data),"{EEE019}NOTA: ahora el actor id:%d tiene el nombre de :%s.", id_seleccionado,InfoActorEdicion[id_seleccionado][a_name]);
						SendClientMessage(playerid,-1,data);
						CallLocalFunction("OnPlayerCommandText", "is", playerid, "/actorfunction");
					}else{
						format(data,sizeof(data),"{FFFFFF}¿Que nombre deseas colocarle al actor id:%d?: \n\
						{A50000}(NOTA: escribe el nombre que quieres que tendra el actor id:%d,  \n\
						debe ser mayor o igual a 4, y menor o igual a 24).", id_seleccionado,id_seleccionado);
						ShowPlayerDialog(playerid, DIALOG_ACTORS+9, DIALOG_STYLE_INPUT, "{FF6800}Sistema de edicion de actores:", data, "aceptar", "cancelar");
					}
				}else{
					format(data,sizeof(data),"{FFFFFF}¿Que nombre deseas colocarle al actor id:%d?: \n\
					{A50000}(NOTA: escribe el nombre que quieres que tendra el actor id:%d,  \n\
	        ...
	        if(response){
				...
				if(!sscanf(inputtext, "s[25]", name_a)){
					if(strlen(name_a)>=4 && strlen(name_a)<=24){
						...
						format(data,sizeof(data),"{FFFFFF}INFO: le haz cambiado el nombre a los %d actorres creados, correctamente!.", cantidad_actores);
						SendClientMessage(playerid,-1,data);
						format(data,sizeof(data),"{EEE019}NOTA: ahora los %d actores tienen el nombre de :%s.", cantidad_actores,name_a);
						SendClientMessage(playerid,-1,data);
						CallLocalFunction("OnPlayerCommandText", "is", playerid, "/actorfunction");
					}else{
						format(data,sizeof(data),"{FFFFFF}¿Que nombre deseas colocarles a los %d actorres creados?: \n\
						{A50000}(NOTA: escribe el nombre que quieres que tendran los %d actor creados,  \n\
						debe ser mayor o igual a 4, y menor o igual a 24).", cantidad_actores,cantidad_actores);
						ShowPlayerDialog(playerid, DIALOG_ACTORS+10, DIALOG_STYLE_INPUT, "{FF6800}Sistema de edicion de actores:", data, "aceptar", "cancelar");
					}
				}else{
					format(data,sizeof(data),"{FFFFFF}¿Que nombre deseas colocarles a los %d actorres creados?: \n\
					{A50000}(NOTA: escribe el nombre que quieres que tendran los %d actor creados,  \n\
	        ...
	        if(response){
			    switch(listitem){
			        case 0:{
						...
						if(!creando_actors[playerid]){
							CallLocalFunction("OnPlayerCommandText", "is", playerid, "/actorfunction");
							return SendClientMessage(playerid,-1,"{919BA4}INFO: usted no se encuentra con el editor de actores encendido!."); }
						if(cantidad_actores<=0){
							CallLocalFunction("OnPlayerCommandText", "is", playerid, "/actorfunction");
							return SendClientMessage(playerid,-1,"{919BA4}INFO: no hay ningun actor creado hasta el momento!."); }

						ShowPlayerDialog(playerid, DIALOG_ACTORS+5, DIALOG_STYLE_MSGBOX, "{FF6800}Sistema de edicion de actores:", "\
						{FFFFFF}¿Deseas seleccionar un id especifico de un actor creado?:", "aceptar", "cancelar");
					}
			        case 1:{
						if(IsPlayerInAnyVehicle(playerid)){
							CallLocalFunction("OnPlayerCommandText", "is", playerid, "/actorfunction");
						...
						if(id_seleccionado<=0){
							CallLocalFunction("OnPlayerCommandText", "is", playerid, "/actorfunction");
							return SendClientMessage(playerid,-1,"{919BA4}INFO: usted no ha seleccionado ningun id para la edicion!."); }
					    ...
					    format(data,sizeof(data),"{EEE019}INFO: le haz aplicado una animacion al actor id:%d, correctamente!.",id_seleccionado);
					    SendClientMessage(playerid,-1,data);
					    CallLocalFunction("OnPlayerCommandText", "is", playerid, "/actorfunction");
					}
			        case 2:{
					    ...
					    SendClientMessage(playerid,-1,"{EEE019}INFO: le haz aplicado una animacion ha todos los actores creados, correctamente!.");
					    CallLocalFunction("OnPlayerCommandText", "is", playerid, "/actorfunction");
					}
			        case 3:{
						if(IsPlayerInAnyVehicle(playerid)){
						...
						format(data,sizeof(data),"{FFFFFF}¿Que nombre deseas colocarle al actor id:%d?:",id_seleccionado);
						ShowPlayerDialog(playerid, DIALOG_ACTORS+9, DIALOG_STYLE_INPUT, "{FF6800}Sistema de edicion de actores:", data, "aceptar", "cancelar");
					}
			        case 4:{
						if(IsPlayerInAnyVehicle(playerid)){
						...
						format(data,sizeof(data),"{FFFFFF}¿Que nombre deseas colocarles a los %d actorres creados?:",cantidad_actores);
						ShowPlayerDialog(playerid, DIALOG_ACTORS+10, DIALOG_STYLE_INPUT, "{FF6800}Sistema de edicion de actores:", data, "aceptar", "cancelar");
					}
					case 5:{
						if(IsPlayerInAnyVehicle(playerid)){
						...
						\t\t{FF6800}>>: selecciona la opcion que prefieras: \n\
						opcion - skin '>>' \n\
					  	opcion - skin '<<' \n\
						opcion - skin 'random'  \n",seleccion_skis);
						ShowPlayerDialog(playerid, DIALOG_ACTORS+12, DIALOG_STYLE_LIST, "{FF6800}Sistema de edicion de actores:", data, "seleccionar", "finalizar");
					}
					case 6:{
						if(IsPlayerInAnyVehicle(playerid)){
							CallLocalFunction("OnPlayerCommandText", "is", playerid, "/actorfunction");
						...
						opcion - skin '>>' \n\
					  	opcion - skin '<<' \n\
						opcion - skin 'random'  \n",seleccion_skis);
						ShowPlayerDialog(playerid, DIALOG_ACTORS+13, DIALOG_STYLE_LIST, "{FF6800}Sistema de edicion de actores:", data, "seleccionar", "finalizar");
					}
					case 7:{
						if(IsPlayerInAnyVehicle(playerid)){
							CallLocalFunction("OnPlayerCommandText", "is", playerid, "/actorfunction");
						...
						ShowPlayerDialog(playerid, DIALOG_ACTORS+14, DIALOG_STYLE_INPUT, "{FF6800}Sistema de edicion de actores:", "\
						{FFFFFF}colocar el color que le quieres colocar al nombre del actor: \n\
						{A50000}(NOTA: el formato del color debe ser en 'hexadecimal', ejemplo: 0xFFFFFFFF \n\
						", "aceptar", "cancelar");
					}
					case 8:{
						if(IsPlayerInAnyVehicle(playerid)){
						...
						ShowPlayerDialog(playerid, DIALOG_ACTORS+15, DIALOG_STYLE_INPUT, "{FF6800}Sistema de edicion de actores:", "\
						{FFFFFF}colocar el color que le quieres colocar al nombre de todos los actores: \n\
						{A50000}(NOTA: el formato del color debe ser en 'hexadecimal', ejemplo: 0xFFFFFFFF \n\
						", "aceptar", "cancelar");
					}
					case 9:{
						...
						SendClientMessage(playerid,-1,"{EEE019}INFO: ahora selecciona a un actor para establecer su id!.");
			    		SendClientMessage(playerid,-1,"{FFFFFF}INFO: para cambiar la camara cuando estas en modo edicion,");
			    		SendClientMessage(playerid,-1,"{FFFFFF}manten la tecla 'BARRA' y mueve el 'mouse' para cambiarla!.");
					}
					case 10:{
						if(IsPlayerInAnyVehicle(playerid)){
							CallLocalFunction("OnPlayerCommandText", "is", playerid, "/actorfunction");
						...
						SelectObject(playerid);
						SendClientMessage(playerid,-1,"{EEE019}INFO: ahora selecciona a un actor para empezar a moverlo!.");
			    		SendClientMessage(playerid,-1,"{FFFFFF}INFO: para cambiar la camara cuando estas en modo edicion,");
			    		SendClientMessage(playerid,-1,"{FFFFFF}manten la tecla 'BARRA' y mueve el 'mouse' para cambiarla!.");
					}
			    }
	        ...
	        if(response){
		        if(strlen(inputtext)==10){
			        if(inputtext[0]=='0' && inputtext[1]=='x'){
			        	...
			        	format(data,sizeof(data),"{FFFFFF}INFO: le haz cambiado el color del nombre al actor id:%d, correctamente!.", id_seleccionado);
						SendClientMessage(playerid,-1,data);
						format(data,sizeof(data),"{A50000}INFO: color cambiado es: {%06x} 0x%08x", HexToInt(InfoActorEdicion[id_seleccionado][color_actor])>>>8, InfoActorEdicion[id_seleccionado][color_actor]);
						SendClientMessage(playerid,-1,data);
			        	CallLocalFunction("OnPlayerCommandText", "is", playerid, "/actorfunction");
			        }else{
						ShowPlayerDialog(playerid, DIALOG_ACTORS+14, DIALOG_STYLE_INPUT, "{FF6800}Sistema de edicion de actores:", "\
	        ...
	        if(response){
		        if(strlen(inputtext)==10){
			        if(inputtext[0]=='0' && inputtext[1]=='x'){
						...
						SendClientMessage(playerid,-1,"{FFFFFF}INFO: le haz cambiado el color del nombre a todos los actores creados, correctamente!.");
						format(data,sizeof(data),"{A50000}INFO: color cambiado es: {%06x} 0x%08x", HexToInt(inputtext)>>>8, HexToInt(inputtext));
						SendClientMessage(playerid,-1,data);
			        	CallLocalFunction("OnPlayerCommandText", "is", playerid, "/actorfunction");
			        }else{
						ShowPlayerDialog(playerid, DIALOG_ACTORS+15, DIALOG_STYLE_INPUT, "{FF6800}Sistema de edicion de actores:", "\
...
public OnPlayerSelectObject(playerid, type, objectid, modelid, Float:fX, Float:fY, Float:fZ){
    ...
    if(type == SELECT_OBJECT_GLOBAL_OBJECT){
    	if(seleccioando_actor[playerid][0]){
    		...
    		id_seleccionado = objectid;
			format(data,sizeof(data),"{FFFFFF}INFO: el id del actor seleccionado es el: id:%d", id_seleccionado);
			SendClientMessage(playerid,-1,data);
    		EditObject(playerid, InfoActorEdicion[id_seleccionado][objeto_select]);
    		SendClientMessage(playerid,-1,"{EEE019}INFO: ahora mueve de posicion al actor con el modo de edicion!.");
    		SendClientMessage(playerid,-1,"{FFFFFF}INFO: para cambiar la camara cuando estas en modo edicion,");
    		SendClientMessage(playerid,-1,"{FFFFFF}manten la tecla 'BARRA' y mueve el 'mouse' para cambiarla!.");
	...
	if(seleccioando_actor[playerid][1]){
	    switch(response){
	        ...
	        case EDIT_RESPONSE_FINAL:{
	        	...
	        	SendClientMessage(playerid,-1,"{EEE019}INFO: haz finalizado la edicion del actor,");
	        	SendClientMessage(playerid,-1,"{EEE019}se le ha establecido la nueva posicion al actor!.");
	        	CallLocalFunction("OnPlayerCommandText", "is", playerid, "/actorfunction");
			}
	        ...
	        case EDIT_RESPONSE_CANCEL:{
		        if(!playerobject){
		        	...
		        	SendClientMessage(playerid,-1,"{EEE019}INFO: haz cancelado la edicion del actor,");
		        	SendClientMessage(playerid,-1,"{EEE019}se le ha establecido la antigua posicion del actor!.");
		        	CallLocalFunction("OnPlayerCommandText", "is", playerid, "/actorfunction");
		        }
			}
		}
...
COMMAND:actorhelp(playerid, params[]){
	new
		data[2024];
	strcat(data,"{FFFFFF}INFO: utiliza las siguientes teclas para la edicion de los actores: \n\n");
	strcat(data,"{FF6800}>>: tecla 'Y' para crear un actor en tu posicion. \n");
	strcat(data,"{FF6800}>>: tecla 'N' para elimirar el actor creado anteriormente. \n");
	strcat(data,"{FF6800}>>: tecla 'C' y 'ENTER' (juntas) para eliminar todos los actores creador hasta el momento. \n");
	strcat(data,"{FF6800}>>: tecla 'C' y 'BARRA' (juntas) para guardar todos los actores creador hasta el momento. \n");
	strcat(data,"{FF6800}>>: tecla 'C' y 'H' (juntas) para iniciar el dialog de ayuda. \n\n");
	strcat(data,"{FFFFFF}INFO: utiliza los siguientes comandos para la edicion de los actores: \n\n");
	strcat(data,"{FF6800}>>: utiliza el comando '/actorfunction' para utilizar la funciones disponibles \n");
	strcat(data,"{FF6800}>>: utiliza el comando '/actor' para encender/apagar el sistema de edicion de actores. \n");
	ShowPlayerDialog(playerid, DIALOG_ACTORS+0, DIALOG_STYLE_MSGBOX, "{FF6800}Sistema de edicion de actores:", data, "cerrar", "mas...");
	return true;
}
...
COMMAND:actorfunction(playerid, params[]){
	...
	new
		data[2024];
	strcat(data,"opcion \t comentario \n");
	// NOTE: change the comment to your preference.
	strcat(data,"{FF6800}>>: {FFFFFF}establecer un id especifico. \t{2C2E2E} created by OTACON.\n");
	strcat(data,"{FF6800}>>: {FFFFFF}establecer una animacion al id especifico. \t{2C2E2E} created by OTACON.\n");
	strcat(data,"{FF6800}>>: {FFFFFF}establecer una animacion a todos los actores. \t{2C2E2E} created by OTACON.\n");
	strcat(data,"{FF6800}>>: {FFFFFF}cambiar el nombre a un actor especifico. \t{2C2E2E} created by OTACON.\n");
	strcat(data,"{FF6800}>>: {FFFFFF}cambiar el nombre a todos los actores. \t{2C2E2E} created by OTACON.\n");
	strcat(data,"{FF6800}>>: {FFFFFF}seleccionador de skins para el id especifico. \t{2C2E2E} created by OTACON.\n");
	strcat(data,"{FF6800}>>: {FFFFFF}seleccionador de skins para todos los actores. \t{2C2E2E} created by OTACON.\n");
	strcat(data,"{FF6800}>>: {FFFFFF}cambiar el color del nombre para el id especifico. \t{2C2E2E} created by OTACON.\n");
	strcat(data,"{FF6800}>>: {FFFFFF}cambiar el color del nombre para todos los actores. \t{2C2E2E} created by OTACON.\n");
	strcat(data,"{FF6800}>>: {FFFFFF}seleccionar un id especifico. \t{2C2E2E} created by OTACON.\n");
	strcat(data,"{FF6800}>>: {FFFFFF}mover de posicion a un actor, seleccionandolo. \t{2C2E2E} created by OTACON.\n");
	ShowPlayerDialog(playerid, DIALOG_ACTORS+11, DIALOG_STYLE_TABLIST_HEADERS,\
	 "{FF6800}Sistema de edicion de actores:", data, "seleccionar", "cancelar");
	return true;
}
...
COMMAND:actor(playerid, params[]){
	...
	if(!creando_actors[playerid]){
		...
		SendClientMessage(playerid,-1,"{447900}INFO: sistema de creacion de actores, encendido.");
		CallLocalFunction("OnPlayerCommandText", "is", playerid, "/actorhelp");
	}else{
		creando_actors[playerid]=false;
		SendClientMessage(playerid,-1,"{A50000}INFO: sistema de creacion de actores, apagado.");
	}
	return true;
}
